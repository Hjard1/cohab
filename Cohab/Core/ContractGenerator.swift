...
enum ContractGenerator {
    ...
    private static func buildSections(household: Household, templates: [String: ContractTemplate] = [:]) -> [(title: String, body: String, kind: ContractSectionKind)] {
        ...
        // § SHARED ASSETS
        sections.append(("\(n).  \(tplTitle(templates, "assets_intro") ?? t(household, no: "FELLES EIENDELER OG EIERSKAP", sv: "REGISTRERADE TILLGÅNGAR OCH ÄGARANDELAR", da: "REGISTREREDE AKTIVER OG EJERANDELE", fi: "REKISTERÖIDYT VARAT JA OMISTUSOSUUDET", de: "ERFASSTE VERMÖGENSWERTE UND EIGENTUMSANTEILE", fr: "ACTIFS ENREGISTRÉS ET QUOTES-PARTS", es: "ACTIVOS REGISTRADOS Y CUOTAS DE PROPIEDAD", en: "RECORDED ASSETS AND OWNERSHIP SHARES"))", {
            n += 1
            ...
            let intro = tpl(templates, "assets_intro") {
                if isNO {
                    ...
                    return "Osapuolet ovat rekisteröineet seuraavat varat cohab-sovellukseen allekirjoitushetkellä ja vahvistavat ilmoitetut omistusosuudet:"
                } else if isDe {
                    return "Die Parteien haben folgende Vermögenswerte in cohab zum Zeitpunkt der Unterzeichnung erfasst und bestätigen die angegebenen Eigentumsanteile:"
                } else if isFr {
                    return "Les parties ont enregistré les actifs suivants dans cohab à la date de signature et confirment les quotes-parts indiquées:"
                } else if isEs {
                    return "Las partes han registrado los siguientes activos en cohab en la fecha de firma y confirman las cuotas de propiedad indicadas:"
                } else {
                    return "The parties have recorded the following assets in cohab at the time of signing and confirm the stated ownership shares:"
                }
            }
        ...
        if household.includeDissolutionClause {
            sections.append(("\(n).  \(tplTitle(templates, "dissolution") ?? t(household, no: "FORDELING VED OPPHØR", sv: "EKONOMISK REGLERING VID FÖRSÄLJNING, UTKÖP ELLER SEPARATION", da: "FORDELING VED OPHØR", fi: "TALOUDELLINEN JÄRJESTELY MYYNNISSÄ, LUNASTUKSESSA TAI EROSSA", de: "WIRTSCHAFTLICHE ABWICKLUNG BEI VERKAUF, ÜBERNAHME ODER TRENNUNG", fr: "RÈGLEMENT ÉCONOMIQUE EN CAS DE VENTE, RACHAT OU SÉPARATION", es: "LIQUIDACIÓN ECONÓMICA EN CASO DE VENTA, ADQUISICIÓN O SEPARACIÓN", en: "SETTLEMENT ON SEPARATION"))", {
                n += 1
        ...
        if household.includeDisposalConsentClause {
            sections.append(("\(n).  \(tplTitle(templates, "disposal_consent") ?? t(household, no: "SAMTYKKE VED SALG OG UTLEIE", sv: "SAMTYCKESKRAV", da: "DISPOSITIONSSAMTYKKE", fi: "LUOVUTUSSUOSTUMUS", de: "VERFÜGUNGSZUSTIMMUNG", fr: "CONSENTEMENT AUX CESSIONS", es: "CONSENTIMIENTO PARA DISPOSICIÓN", en: "JOINT DISPOSAL CONSENT"))", {
                ...
                if isNO {
                    ...
                    return "Kumpikaan osapuoli ei saa myydä, vuokrata, pantata tai muutoin luovuttaa yhteistä omaisuutta ilman toisen kirjallista suostumusta. Ilman suostumusta tehty luovutus on osapuolten välillä sopimusrikkomus; hyvässä uskossa olevan kolmannen osapuolen oikeudet säilyvät."
                } else if isDe {
                    return "Keine Partei darf gemeinsame Vermögenswerte ohne schriftliche Zustimmung der anderen verkaufen, vermieten, verpfänden oder anderweitig veräußern. Verfügungen ohne Zustimmung gelten zwischen den Parteien als Vertragsverletzung; die Rechte gutgläubiger Dritter bleiben unberührt."
                } else if isFr {
                    return "Aucune partie ne peut vendre, louer, hypothéquer ou autrement disposer des actifs communs sans le consentement écrit préalable de l'autre. La disposition sans ce consentement constitue un manquement contractuel entre les parties; les droits des tiers de bonne foi ne sont pas affectés."
                } else if isEs {
                    return "Ninguna parte podrá vender, arrendar, hipotecar o disponer de los activos compartidos sin el consentimiento escrito previo de la otra. La disposición sin dicho consentimiento constituye un incumplimiento contractual entre las partes; los derechos de los terceros de buena fe no se ven afectados."
                } else {
        ...
        sections.append(("\(n).  \(tplTitle(templates, "governing_law") ?? t(household, no: "LOVVALG", sv: "TILLÄMPLIG LAG", da: "LOVVALG", fi: "SOVELLETTAVA LAKI", de: "ANWENDBARES RECHT", fr: "LOI APPLICABLE", es: "LEY APLICABLE", en: "GOVERNING LAW"))", {
            ...
            if isNO {
                ...
                return "Este contrato se rige por la legislación española y, en los territorios con derecho civil propio, por la normativa civil foral aplicable. Las disputas que no puedan resolverse amistosamente se someterán a los tribunales competentes de la jurisdicción donde se encuentre el activo principal compartido."
            } else {
